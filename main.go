...
func (m *MamUpdater) Run() error {
	...
	// Update IP
	return m.updateIP(nil) // nil means use existing cookies
}
...
func (m *MamUpdater) updateIP(initialCookies []*http.Cookie) error {
	targetURL, err := url.Parse(dynSeedBoxUrl)
	if err != nil {
		return fmt.Errorf("failed to parse URL: %w", err)
	}

	// If initial cookies provided (first run), use them
	if initialCookies != nil {
		m.httpClient.Jar.SetCookies(targetURL, initialCookies)
	} else {
...
func (m *MamUpdater) handleFirstRun(mamID string) error {
	...
	if err := m.updateIP(initialCookies); err != nil {
...
func main() {
	...
	updater, err := NewMamUpdater(config)
	if err != nil {
